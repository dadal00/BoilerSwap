<script lang="ts"></script>

<svelte:head>
	<title>Terms of Service & Disclaimer - BoilerSwap</title>
</svelte:head>

<header class="border-b bg-white sticky top-0 z-50">
	<div class="container mx-auto px-4 py-4">
		<div class="flex items-center justify-between">
			<a href="/" class="flex items-center space-x-2">
				<div
					class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-lg flex items-center justify-center"
				>
					<span class="text-white font-bold text-sm">BS</span>
				</div>
				<h1 class="text-2xl font-bold text-gray-900">BoilerSwap</h1>
			</a>

			<nav class="hidden md:flex items-center space-x-6">
				<a href="browse" class="text-gray-600 hover:text-gray-900">Browse</a>
				<a href="post" class="text-gray-600 hover:text-gray-900">Post Item</a>
				<a href="how-it-works" class="text-gray-600 hover:text-gray-900">How It Works</a>
				<a href="about" class="text-gray-600 hover:text-gray-900">About</a>
				<a href="contact" class="text-gray-600 hover:text-gray-900">Contact Us</a>
			</nav>

			<div class="flex items-center space-x-3">
				<a
					href="auth"
					class="border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2 rounded">Sign In</a
				>
				<a
					href="post"
					class="bg-gradient-to-r from-yellow-400 to-amber-500 hover:from-yellow-500 hover:to-amber-600 text-white px-4 py-2 rounded"
					>+ Post Item</a
				>
			</div>
		</div>
	</div>
</header>

<div class="container mx-auto px-6 py-16 max-w-4xl">
	<div class="text-center mb-12">
		<h1 class="text-4xl font-bold text-gray-900 mb-4">Terms of Service & Disclaimer</h1>
		<p class="text-xl text-gray-600">Important information about using BoilerSwap</p>
		<p class="text-sm text-gray-500 mt-2">Last updated: December 2024</p>
	</div>

	<div class="space-y-8">
		<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
			<h2 class="text-xl font-bold text-yellow-800 mb-4 flex items-center">
				<span class="text-2xl mr-3">⚠️</span>
				Important Notice
			</h2>
			<p class="text-yellow-800">
				By using BoilerSwap, you agree to these terms and acknowledge that you understand the risks
				involved in meeting strangers and exchanging items. Please read this document carefully.
			</p>
		</div>

		<div class="bg-white rounded-lg shadow-sm border p-8">
			<h2 class="text-2xl font-bold mb-6 flex items-center">
				<span class="text-2xl mr-3">📋</span>
				Platform Purpose
			</h2>
			<p class="text-gray-700 mb-4">
				BoilerSwap is a free platform designed to facilitate the exchange of items between Purdue
				University students. Our goal is to reduce waste, save money, and build community
				connections.
			</p>
			<p class="text-gray-700 mb-4">This platform is intended for:</p>
			<ul class="list-disc list-inside text-gray-700 space-y-1 ml-4">
				<li>Giving away items you no longer need</li>
				<li>Finding items you need for free</li>
				<li>Reducing waste and promoting sustainability</li>
				<li>Building connections within the Purdue community</li>
			</ul>
		</div>

		<div class="bg-white rounded-lg shadow-sm border p-8">
			<h2 class="text-2xl font-bold mb-6 flex items-center">
				<span class="text-2xl mr-3">👥</span>
				User Responsibilities
			</h2>
			<h4 class="font-semibold text-gray-900 mb-4">As a BoilerSwap user, you agree to:</h4>
			<ul class="list-disc list-inside text-gray-700 space-y-2 ml-4">
				<li>Use only your valid @purdue.edu email address</li>
				<li>Provide accurate descriptions of items</li>
				<li>Be honest about item conditions and any defects</li>
				<li>Respond promptly to messages and inquiries</li>
				<li>Show up on time for arranged pickups</li>
				<li>Treat other users with respect and courtesy</li>
				<li>Report any safety concerns or inappropriate behavior</li>
				<li>Only post items that are legal to possess and transfer</li>
				<li>Not use the platform for commercial purposes</li>
			</ul>
		</div>

		<div class="bg-red-50 border border-red-200 rounded-lg p-8">
			<h2 class="text-2xl font-bold text-red-800 mb-6 flex items-center">
				<span class="text-2xl mr-3">🛡️</span>
				Safety Disclaimer
			</h2>
			<p class="text-red-800 font-semibold mb-4">
				IMPORTANT: BoilerSwap does not verify the identity, background, or intentions of users.
			</p>
			<div class="text-red-700 space-y-2">
				<p>
					<strong>You meet and interact with other users at your own risk.</strong>
				</p>
				<p>Safety recommendations:</p>
				<ul class="list-disc list-inside space-y-1 ml-4">
					<li>Always meet in public, well-lit areas</li>
					<li>Consider bringing a friend to exchanges</li>
					<li>Trust your instincts - if something feels wrong, leave</li>
					<li>Don't share personal information beyond what's necessary</li>
					<li>Use campus locations when possible</li>
					<li>Inform someone about your meeting plans</li>
				</ul>
			</div>
		</div>

		<div class="bg-white rounded-lg shadow-sm border p-8">
			<h2 class="text-2xl font-bold mb-6">Liability Limitations</h2>
			<p class="text-gray-700 mb-4">
				<strong>BoilerSwap and its creators are not responsible for:</strong>
			</p>
			<ul class="list-disc list-inside text-gray-700 space-y-1 ml-4 mb-4">
				<li>The quality, safety, or legality of items posted</li>
				<li>The accuracy of item descriptions</li>
				<li>Failed exchanges or no-shows</li>
				<li>Disputes between users</li>
				<li>Any injuries, damages, or losses during exchanges</li>
				<li>Theft, fraud, or other criminal activity</li>
				<li>Technical issues or platform downtime</li>
				<li>Loss of data or account information</li>
			</ul>
			<p class="text-gray-700 font-semibold">Use of this platform is entirely at your own risk.</p>
		</div>

		<div class="bg-white rounded-lg shadow-sm border p-8">
			<h2 class="text-2xl font-bold mb-6">Prohibited Items</h2>
			<p class="text-gray-700 mb-4">The following items are strictly prohibited on BoilerSwap:</p>
			<ul class="list-disc list-inside text-gray-700 space-y-1 ml-4">
				<li>Weapons of any kind</li>
				<li>Illegal drugs or substances</li>
				<li>Alcohol (users under 21)</li>
				<li>Stolen or counterfeit items</li>
				<li>Items that violate copyright or intellectual property</li>
				<li>Hazardous materials or chemicals</li>
				<li>Live animals</li>
				<li>Items requiring special licenses or permits</li>
				<li>Adult content or materials</li>
				<li>Items that violate Purdue University policies</li>
			</ul>
		</div>

		<div class="bg-blue-50 border border-blue-200 rounded-lg p-8">
			<h2 class="text-2xl font-bold text-blue-800 mb-4">Questions or Concerns?</h2>
			<p class="text-blue-800 mb-4">
				If you have questions about these terms or need to report an issue, please contact us.
			</p>
			<a
				href="contact"
				class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-medium"
				>Contact Support</a
			>
		</div>

		<div class="bg-gray-100 rounded-lg p-6">
			<p class="text-center text-gray-700 font-medium">
				By using BoilerSwap, you acknowledge that you have read, understood, and agree to these
				terms.
			</p>
		</div>
	</div>
</div>
